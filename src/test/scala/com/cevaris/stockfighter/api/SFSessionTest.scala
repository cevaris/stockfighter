...
import com.cevaris.stockfighter.{AccountOrders, Fill, ScalaTest, StockOrder, StockQuote}
...
class SFSessionTest extends ScalaTest {
  ...
  "TradeSessionTest" should {
    ...
    "handle update correctly" in {
      ...
      val session = SFSession(latestQuote = Some(quote))
      ...
      when(so3.direction).thenReturn(DirectionType.Sell)
